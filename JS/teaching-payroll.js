  ...
  try {
    ...
    if (role !== 'all') {
      filteredEmployees = filteredEmployees.filter(emp => {
        ...
        if (!empRole) {
          console.warn(`No role mapping for employee: ${emp.id}, designation: ${emp.designation}`);
        }
        return normalizedRole === role;
      });
    }

    // Filter by month and/or year (employees hired on or before the selected month/year)
    if (month || year) {
      filteredEmployees = filteredEmployees.filter(emp => {
        ...
        if (year && hireDate.getFullYear() > parseInt(year)) {
          return false;
        }
        // Check month if specified
        if (month && hireDate.getMonth() + 1 > parseInt(month)) {
          return false;
        }
        return true;
      });
    ...
    tbody.innerHTML = payrollData.length
      ? payrollData.map(item => {
          ...
          return `
            <tr>
              <td>${item.id}</td>
              <td><img src="${item.profilePic}" alt="Profile" style="width: 40px; height: 40px; border-radius: 50%;"></td>
              <td>${item.name}</td>
...
async function renderPayrollChart(role = 'all', month = '', year = '') {
  try {
    ...
    const labels = role === 'all' ? ['Senior Faculty', 'Junior Faculty'] : [role === 'senior_faculty' ? 'Senior Faculty' : 'Junior Faculty'];
    const data = role === 'all' ? [summaryData.senior_faculty, summaryData.junior_faculty] : [summaryData[role]];
    const backgroundColor = role === 'all' ? [primaryColor, secondaryColor] : [role === 'senior_faculty' ? primaryColor : secondaryColor];
